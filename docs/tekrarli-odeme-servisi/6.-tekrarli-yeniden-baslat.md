...
<mark style="color:orange;">`PUT`</mark> `{{Base Adres}}/api/paywall/recurring/resubscribe`

{% hint style="info" %}
Önemli: **Tekrar Ödeme Yeniden Başlatma** servisini kullanabilmeniz için 'Header' alanında '**apikeypublic**' ve '**apiclientpublic**' parametrelerini göndermeniz gerekmektedir.\
\
[<mark style="color:green;">**PaymentAPI Adresi**</mark>](https://developer.paywall.one/ortam)
{% endhint %}

{% hint style="warning" %}
Üyeliği tekrar başlatmak istediğinizde tekrarlı ödeme üyeliği içerisinde aktif bir kart bulunmak zorundadır. Mevcut kartın silinmiş olması durumunda Paywall hata mesajı döner.
{% endhint %}
